...
class Settings(BaseSettings):
    ...
    ws_pool_size: int = Field(default=8, ge=1, le=32, description="WebSocket pool size per DC")
    ws_pool_max_age: float = Field(default=120.0, ge=30.0, description="Max age of pooled connection (seconds)")
    ws_connect_timeout: float = Field(default=5.0, ge=1.0, description="WebSocket connect timeout")
    ...
    # Telegram DC IPs (format: "dc_id:ip_address")
    dc_ips: List[str] = Field(
        default=["2:149.154.167.220", "4:149.154.167.220"],
        description="Telegram DC IP mappings"
    )
